...
const openModal = async (id: string | null = null, disabled: boolean = false) => {
  ...
  titleModal.value = id ? "Editar Nivel" : "Crear Nivel"
...
const fetchDataForm = async () => {
  ...
  const url = form.value.id ? `/level/${form.value.id}/edit` : `/level/create`
...
const submitForm = async () => {
  ...
  if (validation?.valid) {
    ...
    const url = form.value.id ? `/level/update/${form.value.id}` : `/level/store`
            ...
            <VRow>
              <VCol cols="12">
                <AppTextField :requiredField="true" clearable :disabled="disabledFiledsView" label="Nombre"
                  :rules="[requiredValidator]" v-model="form.level" :error-messages="errorsBack.level"
                  @input="errorsBack.level = ''" />
              </VCol>
            </VRow>
          </VForm>
        </VCardText>
