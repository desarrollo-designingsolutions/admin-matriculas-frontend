...
const openModal = async (id: string | null = null, disabled: boolean = false) => {
  ...
  titleModal.value = id ? "Editar Periodo" : "Crear Periodo"
...
const fetchDataForm = async () => {
  ...
  const url = form.value.id ? `/period/${form.value.id}/edit` : `/period/create`
...
const submitForm = async () => {
  ...
  if (validation?.valid) {
    ...
    const url = form.value.id ? `/period/update/${form.value.id}` : `/period/store`
      ...
      <VCard :loading="isLoading" :disabled="isLoading" class="w-100">
        ...
        <VCardText>
          <VForm ref="refForm" @submit.prevent="() => { }">
            <VRow>
              <VCol cols="12">
                <AppTextField :requiredField="true" clearable :disabled="disabledFiledsView" label="Periodo"
                  :rules="[requiredValidator]" v-model="form.period" :error-messages="errorsBack.period"
                  @input="errorsBack.period = ''" />
              </VCol>
            </VRow>
          </VForm>
        </VCardText>
