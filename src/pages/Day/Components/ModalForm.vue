...
const fetchDataForm = async () => {
  ...
  const url = form.value.id ? `/day/${form.value.id}/edit` : `/day/create`
...
const submitForm = async () => {
  ...
  if (validation?.valid) {
    ...
    const url = form.value.id ? `/day/update/${form.value.id}` : `/day/store`
      ...
      <VCard :loading="isLoading" :disabled="isLoading" class="w-100">
        ...
        <VCardText>
          <VForm ref="refForm" @submit.prevent="() => { }">
            <VRow>
              <VCol cols="12">
                <AppTextField :requiredField="true" clearable :disabled="disabledFiledsView" label="Dia"
                  :rules="[requiredValidator]" v-model="form.day" :error-messages="errorsBack.day"
                  @input="errorsBack.day = ''" />
              </VCol>
            </VRow>
          </VForm>
        </VCardText>
